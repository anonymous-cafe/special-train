...
export function PipelineSettings({
  ...
  async function handleSave() {
    ...
    const stageRows = localStages.map((s, i) => ({
      ...
      color: s.color,
      position: i,
      default_probability: s.default_probability ?? 20,
  ...
  async function handleAddStage() {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        default_probability: Math.min(95, Math.max(5, (localStages.length + 1) * 20)),
        outcome: 'open',
      })
      .select()
      .single();
    ...
    >
      ...
      <div className="min-w-0 flex-1 space-y-1.5">
        <Input
          ...
          className="h-7 w-full border-transparent bg-transparent text-sm text-white focus:border-slate-600"
        />
        <div className="grid grid-cols-[88px_1fr] gap-1.5">
          <div className="relative">
            <input
              ...
              min={0}
              max={100}
              value={stage.default_probability ?? 20}
              onChange={(e) => onProbabilityChange(Math.max(0, Math.min(100, Number(e.target.value) || 0)))}
              className="h-7 w-full rounded-md border border-slate-700 bg-slate-900 px-2 pr-6 text-xs text-white outline-none focus:border-primary"
          ...
          <select
            ...
            onChange={(e) => onOutcomeChange(e.target.value as 'open' | 'won' | 'lost')}
            className="h-7 rounded-md border border-slate-700 bg-slate-900 px-2 text-xs text-slate-300 outline-none focus:border-primary"
