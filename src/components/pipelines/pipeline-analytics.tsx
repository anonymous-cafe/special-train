...
export function PipelineAnalytics({ stages, deals }: PipelineAnalyticsProps) {
  ...
  const stats = useMemo(() => {
    ...
    const weightedValue = openDeals.reduce((sum, d) => {
      const stage = stageById.get(d.stage_id) as PipelineStage | undefined;
      const probability = d.probability ?? stage?.default_probability ?? 0;
      return sum + Number(d.value || 0) * (Math.max(0, Math.min(100, probability)) / 100);
    }, 0);
    ...
    <TooltipProvider>
      <div className="grid grid-cols-2 gap-3 rounded-xl border border-slate-800 bg-slate-900/60 p-4 sm:grid-cols-3 xl:grid-cols-6">
        ...
        <Metric
          ...
          tooltip="Expected revenue: each open deal's value × its saved probability. Stage defaults are applied automatically when a deal moves. Lost deals are excluded."
        />
        <Metric
