...
const STEP_META: Record<AutomationStepType, StepMeta> = {
  ...
  ai_reply: { label: "AI Knowledge Reply", icon: Sparkles, border: "border-l-violet-500" },
...
const TRIGGER_OPTIONS: { value: AutomationTriggerType; label: string; hint: string }[] = [
  ...
  { value: "no_response_received", label: "No Customer Response", hint: "When an outbound message has no customer reply for a delay" },
]
...
function blankConfig(type: AutomationStepType): Record<string, unknown> {
  switch (type) {
    ...
    case "ai_reply":
      return { instruction: "Answer using the knowledge base and recent conversation. If unsure, ask a clarifying question." }
          ...
          <div className="space-y-3 border-t border-slate-800 px-4 py-3">
            ...
            {type === "time_based" && (
              <div className="space-y-2">
                ...
                <FieldBlock label="Timezone (optional)">
                  <Input
                    placeholder="Asia/Dhaka (defaults to UTC)"
            ...
            {type === "no_response_received" && (
              <div className="grid grid-cols-2 gap-2">
                <FieldBlock label="Wait">
                  <Input
                    type="number"
                    min={1}
                    value={(config.amount as number) ?? 1}
                    onChange={(e) =>
                      onConfigChange({ ...config, amount: Math.max(1, Number(e.target.value)) })
                    }
                    className="bg-slate-800 text-white"
                  />
                </FieldBlock>
...
function StepEditor({
  ...
  switch (step.step_type) {
    ...
    case "ai_reply":
      return (
        <FieldBlock label="AI instruction (optional)">
          <Textarea
            value={(cfg.instruction as string) ?? ""}
            onChange={(e) => set({ instruction: e.target.value })}
            placeholder="Use the knowledge base, answer briefly, and ask for missing order details."
            className="min-h-20 bg-slate-800 text-white"
          />
          <p className="mt-1 text-[11px] text-slate-500">Requires Patch 10 AI provider settings and active knowledge-base articles.</p>
        </FieldBlock>
      )
    case "close_conversation":
      return (
