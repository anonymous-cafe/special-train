...
import { Search, ChevronDown, AlarmClock, Flag } from "lucide-react";
...
type ConversationFilter = ConversationStatus | "all" | "unread" | "snoozed" | "urgent";

const FILTER_OPTIONS: { label: string; value: ConversationFilter }[] = [
  { label: "All", value: "all" },
...
export function ConversationList({
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "unread") {
      ...
      result = result.filter((c) => Boolean(c.snoozed_until && new Date(c.snoozed_until).getTime() > Date.now()));
    } else if (filter !== "all") {
      result = result.filter((c) => c.status === filter);
    } else {
      // Keep actively snoozed conversations out of the default inbox view.
      result = result.filter((c) => !c.snoozed_until || new Date(c.snoozed_until).getTime() <= Date.now());
    }
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex items-center justify-between gap-2">
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            {conversation.snoozed_until && new Date(conversation.snoozed_until).getTime() > Date.now() && (
              <AlarmClock className="h-3 w-3 text-violet-400" aria-label="Snoozed" />
            )}
            {(conversation.priority === "high" || conversation.priority === "urgent") && (
              <Flag className={cn("h-3 w-3", conversation.priority === "urgent" ? "text-red-400" : "text-amber-400")} aria-label={`${conversation.priority} priority`} />
            )}
