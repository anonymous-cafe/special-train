...
import { ConversationAIAssistant } from "@/components/ai/conversation-ai-assistant";
...
const PRIORITY_OPTIONS: { label: string; value: ConversationPriority; color: string }[] = [
  { label: "Low", value: "low", color: "text-slate-400" },
  { label: "Normal", value: "normal", color: "text-blue-400" },
...
export function MessageThread({
  ...
  // Owner-scoped CRM team directory used for assignment. Patch-13 will
  // extend this into shared authenticated workspace RBAC without weakening
  // the current user-scoped RLS model.
  useEffect(() => {
    let cancelled = false;
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (cancelled) return;
        if (error) {
          console.error("Failed to fetch CRM agents:", error);
          return;
        }
  ...
  const handleSendMedia = useCallback(
    async (item: MediaLibraryItem, replyToId?: string) => {
      if (!conversation) return;
      const tempId = `temp-${Date.now()}`;
      const optimisticMsg: Message = {
        id: tempId,
        ...
        content_text: item.caption || item.name,
      ...
      try {
        ...
        const payload = await res.json().catch(() => ({}));
        if (!res.ok) {
          toast.error(`Failed to send media: ${payload?.error || `HTTP ${res.status}`}`);
          onUpdateMessage(tempId, { status: "failed" });
          return;
        }
        onUpdateMessage(tempId, { status: "sent" });
      } catch (err) {
        toast.error(`Failed to send media: ${err instanceof Error ? err.message : "network error"}`);
        onUpdateMessage(tempId, { status: "failed" });
      }
    },
    [conversation, onNewMessage, onUpdateMessage],
  ...
  const handlePriorityChange = useCallback(
    async (priority: ConversationPriority) => {
      ...
      onConversationPatch?.(conversation.id, { priority });
    },
  ...
  const handleSnooze = useCallback(
    async (hours: number | null) => {
      ...
      onConversationPatch?.(conversation.id, {
        snoozed_until: snoozed_until ?? undefined,
      });
      toast.success(hours ? `Snoozed for ${hours}h` : "Snooze cleared");
    },
    [conversation, onConversationPatch],
  );
  ...
  const currentAssignee = agents.find((agent) => agent.id === assignedAgentId);
  const assignLabel = assignedAgentId
    ? (currentAssignee?.name ?? "Assigned")
    : "Assign";
      ...
      <div className="flex items-center justify-between gap-2 border-b border-slate-800 bg-slate-900 px-3 py-3 sm:px-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <ConversationAIAssistant
            ...
            onSendReply={async (text) => handleSend(text)}
          />
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className={cn(
              ...
              currentPriority?.color ?? "text-slate-400",
            )}>
              <Flag className="h-3 w-3" />
              <span className="hidden md:inline">{currentPriority?.label ?? "Priority"}</span>
              <ChevronDown className="h-3 w-3" />
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="border-slate-700 bg-slate-800">
              {PRIORITY_OPTIONS.map((opt) => (
                <DropdownMenuItem key={opt.value} onClick={() => void handlePriorityChange(opt.value)} className={cn("text-sm", opt.color)}>
                  {opt.label}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="border-slate-700 bg-slate-800">
              <DropdownMenuItem onClick={() => void handleSnooze(1)} className="text-slate-300">1 hour</DropdownMenuItem>
              <DropdownMenuItem onClick={() => void handleSnooze(4)} className="text-slate-300">4 hours</DropdownMenuItem>
              <DropdownMenuItem onClick={() => void handleSnooze(24)} className="text-slate-300">Tomorrow</DropdownMenuItem>
              <DropdownMenuSeparator className="bg-slate-700" />
              <DropdownMenuItem onClick={() => void handleSnooze(null)} className="text-slate-400">Clear snooze</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
            ...
            >
              {agents.length === 0 ? (
                <DropdownMenuItem disabled className="text-sm text-slate-500">
                  No CRM agents. Add them in Settings → Team.
                </DropdownMenuItem>
              ) : (
                agents.map((agent) => {
                  const isSelected = agent.id === assignedAgentId;
                  return (
                    <DropdownMenuItem
                      key={agent.id}
                      onClick={() => handleAssignChange(agent.id)}
                      className={cn(
                        "text-sm",
                    ...
                    >
                      <span className="flex-1">
                        {agent.name}
                        <span className="ml-1 text-[10px] text-slate-500">{agent.role}</span>
                      </span>
