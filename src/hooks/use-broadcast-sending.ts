...
interface UseBroadcastSendingReturn {
  ...
  createScheduledBroadcast: (payload: BroadcastPayload, scheduledAt: string) => Promise<string>;
...
export function useBroadcastSending(): UseBroadcastSendingReturn {
  ...
  async function resolveAudience(audience: AudienceConfig, frequencyCapHours = 24): Promise<Contact[]> {
    ...
    // Marketing safety: never include contacts that explicitly opted out.
    contacts = contacts.filter((c) => !c.whatsapp_opt_out);

    // Frequency cap prevents repeatedly hitting the same contact within the
    // configured window. A cap of 0 disables the cooldown.
    if (frequencyCapHours > 0) {
      const cutoff = Date.now() - frequencyCapHours * 3_600_000;
      contacts = contacts.filter((c) => {
        if (!c.last_broadcast_at) return true;
        return new Date(c.last_broadcast_at).getTime() <= cutoff;
      });
    }
  ...
  async function createAndSendBroadcast(payload: BroadcastPayload): Promise<string> {
    ...
    try {
      ...
      setProgress(5);
      const contacts = await resolveAudience(payload.audience, payload.frequencyCapHours ?? 24);
      ...
      const { data: broadcast, error: broadcastError } = await supabase
        ...
        .insert({
          ...
          replied_count: 0,
          failed_count: 0,
          frequency_cap_hours: payload.frequencyCapHours ?? 24,
          retry_limit: payload.retryLimit ?? 2,
          ...
          for (const recipient of batch) {
            ...
            if (result.status === 'sent') {
              ...
              if (recipient.contact?.id) {
                await supabase
                  .from('contacts')
                  .update({ last_broadcast_at: sentAt })
                  .eq('id', recipient.contact.id);
              }
            } else {
              failedCount++;
              await supabase
                ...
                .update({
                  ...
                  next_retry_at: (payload.retryLimit ?? 2) > 0 ? new Date(Date.now() + 5 * 60_000).toISOString() : null,
                })
                .eq('id', recipient.id);
            }
          }
      ...
      setProgress(95);
      const canAutoRetry = failedCount > 0 && (payload.retryLimit ?? 2) > 0;
      const finalStatus = canAutoRetry
        ? 'sending'
      ...
      await supabase
        ...
        .update({
          status: finalStatus,
          completed_at: finalStatus === 'sending' ? null : new Date().toISOString(),
        })
        .eq('id', broadcast.id);
  ...
  async function createScheduledBroadcast(
    ...
    try {
      ...
      const contacts = await resolveAudience(
        payload.audience,
        payload.frequencyCapHours ?? 24,
      );
      if (contacts.length === 0) {
        throw new Error('No eligible contacts found for this audience.');
      }
      ...
      const { data: broadcast, error: broadcastError } = await supabase
        ...
        .single();
      if (broadcastError || !broadcast) {
        throw new Error(`Failed to schedule broadcast: ${broadcastError?.message ?? 'unknown error'}`);
      }
      ...
      for (let i = 0; i < recipientRows.length; i += INSERT_BATCH_SIZE) {
        ...
        if (error) {
          ...
          throw new Error(`Failed to create scheduled recipients: ${error.message}`);
        }
      }
      setProgress(100);
