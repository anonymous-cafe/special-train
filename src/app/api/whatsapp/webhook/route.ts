...
interface WhatsAppWebhookEntry {
  ...
  changes: Array<{
    value: {
      ...
      history?: Array<{
        metadata?: { phase?: string; chunk_order?: number; progress?: string | number }
...
export async function POST(request: Request) {
  ...
  // Ack Meta immediately, but keep the work attached to the request
  // lifecycle. Plain fire-and-forget promises can be terminated as soon
  // as a serverless runtime sends the response; Next's `after()` keeps
  // this callback alive for the deployment's post-response window.
  after(async () => {
    try {
...
async function processWebhook(body: { entry?: WhatsAppWebhookEntry[] }) {
  ...
  for (const entry of body.entry) {
    for (const change of entry.changes) {
      ...
      // WhatsApp Business App coexistence webhooks are separate fields.
      // Route them before the ordinary `messages` payload so app-originated
      // echoes are never mistaken for customer inbound messages.
      if (change.field === 'smb_app_state_sync') {
        await handleCoexistenceContactSync(value)
        continue
      }
      if (change.field === 'smb_message_echoes') {
        await handleCoexistenceMessageEchoes(value)
        continue
      }
      ...
      if (change.field === 'account_update') {
        await handleCoexistenceAccountUpdate(entry.id, value)
        continue
      }
...
async function handleCoexistenceAccountUpdate(
  entryWabaId: string,
  value: WhatsAppWebhookEntry['changes'][number]['value'],
) {
  const wabaId = value.waba_info?.waba_id || entryWabaId
  if (!wabaId || !value.event) return
  ...
  const oldMetadata = config.connection_metadata && typeof config.connection_metadata === 'object'
    ? config.connection_metadata
    : {}
  const disconnected = value.event === 'PARTNER_REMOVED' || value.event === 'ACCOUNT_OFFBOARDED'
  const reconnected = value.event === 'ACCOUNT_RECONNECTED'

  await supabaseAdmin().from('whatsapp_config').update({
    ...(disconnected ? { status: 'disconnected', embedded_signup_status: 'disconnected' } : {}),
    ...(reconnected ? { status: 'connected', embedded_signup_status: 'connected' } : {}),
    connection_metadata: {
      ...oldMetadata,
      last_account_update: {
        event: value.event,
...
async function getCoexistenceConfig(phoneNumberId: string) {
  const { data, error } = await supabaseAdmin()
    ...
    .maybeSingle()
  if (error || !data) {
    console.error('[coexistence] No config for phone_number_id:', phoneNumberId, error?.message)
    return null
  }
  return data
}

async function metaMessageAlreadyStored(messageId: string): Promise<boolean> {
...
function allowedContentType(type: string): string {
...
async function handleCoexistenceContactSync(value: WhatsAppWebhookEntry['changes'][number]['value']) {
  const phoneNumberId = value.metadata?.phone_number_id
  if (!phoneNumberId || !value.state_sync?.length) return
  const config = await getCoexistenceConfig(phoneNumberId)
  if (!config) return

  for (const item of value.state_sync) {
    if (item.type && item.type !== 'contact') continue
    const rawPhone = item.contact?.phone_number
    if (!rawPhone) continue
    const phone = normalizePhone(rawPhone)
    const name = item.contact?.full_name || item.contact?.first_name || phone

    if (item.action === 'remove') {
      // Do not delete CRM history when a phone contact is removed from the
      // Business App. Record the sync event instead.
      const existing = await findOrCreateContact(config.user_id, phone, name)
      if (existing) {
        await supabaseAdmin().from('crm_activity').insert({
          user_id: config.user_id,
          contact_id: existing.contact.id,
          entity_type: 'contact',
          entity_id: existing.contact.id,
          action: 'whatsapp_contact_removed',
          title: 'Removed from WhatsApp Business App contacts',
          details: { source: 'smb_app_state_sync' },
        })
      }
      continue
    }

    const outcome = await findOrCreateContact(config.user_id, phone, name)
    if (outcome) {
      await supabaseAdmin().from('crm_activity').insert({
        user_id: config.user_id,
        contact_id: outcome.contact.id,
        entity_type: 'contact',
        entity_id: outcome.contact.id,
        action: 'whatsapp_contact_synced',
        title: outcome.wasCreated ? 'Imported from WhatsApp Business App' : 'Synced from WhatsApp Business App',
        details: { source: 'smb_app_state_sync', action: item.action || 'add' },
      })
    }
  }
}

async function handleCoexistenceMessageEchoes(value: WhatsAppWebhookEntry['changes'][number]['value']) {
  const phoneNumberId = value.metadata?.phone_number_id
  ...
  let accessToken = ''
  try { accessToken = decrypt(config.access_token) } catch { return }

  for (const message of value.message_echoes) {
    if (!message.id || await metaMessageAlreadyStored(message.id)) continue
    const recipientPhone = message.to ? normalizePhone(message.to) : ''
    if (!recipientPhone) continue

    const contactOutcome = await findOrCreateContact(config.user_id, recipientPhone, recipientPhone)
    if (!contactOutcome) continue
    const conversation = await findOrCreateConversation(config.user_id, contactOutcome.contact.id)
    ...
    const { error } = await supabaseAdmin().from('messages').insert({
      ...
      sender_type: 'agent',
      sender_id: null,
      content_type: allowedContentType(message.type),
...
async function handleCoexistenceHistory(value: WhatsAppWebhookEntry['changes'][number]['value']) {
  const phoneNumberId = value.metadata?.phone_number_id
  ...
  let accessToken = ''
  try { accessToken = decrypt(config.access_token) } catch { return }
  const businessPhone = normalizePhone(value.metadata?.display_phone_number || config.business_phone || '')

  for (const chunk of value.history) {
    for (const thread of chunk.threads || []) {
      const threadPhone = thread.id ? normalizePhone(thread.id) : ''
      if (!threadPhone) continue
      const contactOutcome = await findOrCreateContact(config.user_id, threadPhone, threadPhone)
      if (!contactOutcome) continue
      const conversation = await findOrCreateConversation(config.user_id, contactOutcome.contact.id)
      if (!conversation) continue
      ...
      for (const message of thread.messages || []) {
        ...
        const parsed = await parseMessageContent(message, accessToken)
        const createdAt = new Date(Number(message.timestamp) * 1000).toISOString()
        const fromBusiness = businessPhone && phonesMatch(message.from, businessPhone)
        const rawStatus = message.history_context?.status || (fromBusiness ? 'sent' : 'delivered')
        const status = ['sending', 'sent', 'delivered', 'read', 'failed'].includes(rawStatus) ? rawStatus : (fromBusiness ? 'sent' : 'delivered')
        const { error } = await supabaseAdmin().from('messages').insert({
          conversation_id: conversation.id,
        ...
        if (error) {
          console.error('[coexistence] history message insert failed:', error.message)
          continue
        }
        if (!newestAt || createdAt > newestAt) {
          newestAt = createdAt
          newestText = parsed.contentText || `[${message.type}]`
        }
      }
    ...
    if (progress !== undefined) {
      const previousMetadata = (config.connection_metadata && typeof config.connection_metadata === 'object')
        ? config.connection_metadata
        : {}
      await supabaseAdmin().from('whatsapp_config').update({
        connection_metadata: {
          ...
          history_sync: {
            ...
            chunk_order: chunk.metadata?.chunk_order ?? null,
            progress,
            updated_at: new Date().toISOString(),
          },
        },
...
async function processMessage(
  ...
  // Patch 07: honor common WhatsApp marketing opt-out / opt-in keywords.
  // This only changes broadcast eligibility; normal 1:1 support replies remain available.
  if (message.type === 'text' && contentText) {
    const command = contentText.trim().toUpperCase()
    const optOutWords = new Set(['STOP', 'UNSUBSCRIBE', 'CANCEL', 'END', 'QUIT'])
    const optInWords = new Set(['START', 'SUBSCRIBE', 'UNSTOP'])
    if (optOutWords.has(command)) {
      await supabaseAdmin().from('contacts').update({
        whatsapp_opt_out: true,
        opt_out_at: new Date().toISOString(),
        opt_out_reason: `keyword:${command.toLowerCase()}`,
      }).eq('id', contactRecord.id)
    } else if (optInWords.has(command)) {
      await supabaseAdmin().from('contacts').update({
