...
export default function BroadcastDetailPage() {
  ...
  async function handleRetryFailed() {
    setRetrying(true);
    try {
      const res = await fetch(`/api/broadcasts/${broadcastId}/retry`, { method: 'POST' });
      const body = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(body?.error || `HTTP ${res.status}`);
      toast.success(`Retry processed ${body?.processed ?? 0} recipient(s)`);
      const supabase = createClient();
      ...
      if (bcRes.data) setBroadcast(bcRes.data as Broadcast);
      if (recRes.data) setRecipients(recRes.data as BroadcastRecipient[]);
    } catch (error) {
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-3 text-sm text-slate-400">
              ...
              {broadcast.scheduled_at && (
                <>
                  <span>-</span>
                  <span className="inline-flex items-center gap-1"><CalendarClock className="h-3.5 w-3.5" />{new Date(broadcast.scheduled_at).toLocaleString()}</span>
                </>
              )}
            </div>
          </div>
