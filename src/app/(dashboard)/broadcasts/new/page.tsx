...
import { Step4ScheduleSend, type BroadcastSendOptions } from '@/components/broadcasts/step4-schedule-send';
...
export default function NewBroadcastPage() {
  ...
  async function handleSend(options: BroadcastSendOptions) {
    if (!template) return;
    ...
    const payload = {
      ...
      variables,
      frequencyCapHours: options.frequencyCapHours,
      retryLimit: options.retryLimit,
    };

    try {
      const broadcastId = options.mode === 'scheduled' && options.scheduledAt
        ? await createScheduledBroadcast(payload, options.scheduledAt)
        : await createAndSendBroadcast(payload);
      toast.success(options.mode === 'scheduled' ? 'Broadcast scheduled' : 'Broadcast started');
