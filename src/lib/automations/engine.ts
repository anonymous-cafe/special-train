 ...
 */
/** Run one active automation by id. Used by the scheduled/no-response cron
 * dispatcher after it has claimed a dedupe receipt. */
export async function runAutomationById(input: {
  ...
  const { data, error } = await db
    ...
    .maybeSingle()
  if (error || !data) {
    if (error) console.error('[automations] run by id fetch failed:', error)
    return false
  }
  await executeAutomation(data as Automation, {
    userId: input.userId,
  ...
  switch (step.step_type) {
    ...
    case 'ai_reply': {
      ...
      const { data: rows, error: rowsError } = await db
        ...
        .limit(24)
      if (rowsError) throw new Error(`AI transcript failed: ${rowsError.message}`)
      ...
      const result = await completeWithAI({
        system: 'You are a WhatsApp customer-service assistant. Never invent business facts. Prefer the supplied knowledge base and keep the reply concise.',
        user: `${cfg.instruction ?? 'Answer the latest customer message using the knowledge base.'}\n\nConversation:\n${transcript}\n\nKnowledge base:\n${knowledge || '(none)'}`,
        temperature: 0.3,
      })
      ...
      await db.from('ai_activity_logs').insert({
        user_id: args.automation.user_id,
        contact_id: args.contactId,
      ...
      return `AI reply sent via Meta (${sent.whatsapp_message_id})`
    }
