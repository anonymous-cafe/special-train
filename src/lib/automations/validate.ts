...
export function validateTriggerForActivation(
  ...
  } else if (triggerType === 'no_response_received') {
    const amount = Number(cfg.amount)
    if (!Number.isFinite(amount) || amount <= 0) {
      issues.push({ path: 'trigger.amount', message: 'no-response delay must be greater than 0' })
    }
    if (!['minutes', 'hours', 'days'].includes(String(cfg.unit))) {
      issues.push({
        path: 'trigger.unit',
        message: 'no-response unit must be minutes, hours, or days',
      })
    }
  }
